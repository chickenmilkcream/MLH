...
class AvlNode
{
    ...
    int       height;
    int value;
    AvlNode( const Comparable & theElement, AvlNode *lt, AvlNode *rt, int h = 0, int val=69 )
            : element( theElement ), left( lt ), right( rt ), height( h ), value(val) { }
...
public:
    ...
    void makeEmpty( );
    void insert( const Comparable & key, int val);
...
private:
    ...
    void insert( const Comparable & x, AvlNode<Comparable> * & t, int val ) const;
